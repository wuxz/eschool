...
public class AVTransmit {
    ...
    public AVTransmit(MediaLocator locator,
			 String ipAddress,
			 String pb,
			 Format format,String type,SystemControl sysControl) {
	...
	if (result != null) {
        ...
        try{
              ...
              //PlayerWindow playerWindow=new PlayerWindow(processor,this.systemControl);//by yan
              playerWindow=new PlayerWindow(processor,this.systemControl);
               playerWindow.initialize();
               if(type!=null&&type.equals("SCREEN"))
                  playerWindow.setVisible(true);//updated by yan true
               else
	...
	synchronized (this) {
		...
		processor.close();
		processor = null;
            this.playerWindow.dispose();//by yan
	...
	for (int i = 0; i < tracks.length; i++) {
		...
		if (supported.length > 0) {
		    //if (supported[0] instanceof VideoFormat) {
                    if (supported[supported.length-1] instanceof VideoFormat) {
	...
	for (int i = 0; i < pbss.length; i++) {
		...
		// if you use AVTransmit in conjunction with JMStudio.
...
*/
	...
	Player player;
      private SystemControl sysControl;
        PlayerWindow(Player p,SystemControl sysControl) {
	    player = p;
	...
	public void initialize() {
          this.addWindowListener(new java.awt.event.WindowAdapter()
          {
              ...
              {
                  //calling rmi function
                  sysControl.communication.endDemonstrate();
                  this_windowClose(e);
              }
          });
      ...
      public void this_windowClose(WindowEvent e){
          close();
      }
    ...
    {
      if (processor==null) System.out.println("av: processor is null");
      return processor;
    }
