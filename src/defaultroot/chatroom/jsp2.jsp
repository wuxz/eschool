...
<FORM name="Jsp2Form"  method="post"  action="jsp2.jsp">
  <table border="0" width="100%" align="center" cellspacing="0" cellpadding="0">
    <tr> 
      <td width="6%" height="42"><%-- Get talkwith for jsp3.jsp  --%>
<%
	String   withstr=new String("ALL");
	String    Withsen =(String)request.getParameter("talkwith");
    if(Withsen==null)  
 		session.setAttribute("talkwith",withstr); 
    else 
		session.setAttribute("talkwith",Withsen);	
  
	String  withstr2= (String)session.getAttribute("talkwith");
...
%> 
      ...
      <td width="69%" height="42"> 
        ...
        &nbsp <%-- Get nameValue from chat.jsp (username=session.getAttribute("Name"))  --%> 
...
<% 
	...
	session.setAttribute("alonetalk",AloneTalk); 
%> </td>
  ...
  <table border="0" width="90%" align="center">
	...
	testname=(String)session.getAttribute("name");
...
%> 
  ...
  <table border="0" width="60%" align="center">
	...
	Withsen =(String)session.getAttribute("talkwith");
	if((TalkWith!=null)&&(!(Withsen.compareTo(TalkWith)==0)))
	  session.setAttribute("talkwith",TalkWith);      
	...
	String str13=(String)session.getAttribute("alonetalk");
	String str14=new String("yes");
	
	String withstr11=(String)session.getAttribute("talkwith");
	...
	else if(TalkMessage.equals(session.getAttribute("TalkMessage")))
	...
	{  
		session.setAttribute("TalkMessage",TalkMessage);
		...
		fontstyle=(String)session.getAttribute("fontstyle");
		if(fontstyle!=null)
			TalkMessage="<"+fontstyle+">"+TalkMessage+"</"+fontstyle+">";
	
		fontcolor=(String)session.getAttribute("fontcolor");
		if(fontcolor!=null)
		{
		...
		else
		{
			...
			{ 
				...
				Message.addElement(session.getAttribute("alonetalk")); 
				Message.addElement(session.getAttribute("name"));
				Message.addElement(session.getAttribute("talkwith"));
		...
		out.println("提示信息：");
		Integer count = null;
		count = (Integer) session.getAttribute("change");
		if (count == null)
		count = new Integer(0);
		count = new Integer(count.intValue() + 1);
		session.setAttribute("change", count);
	}
%> <font color=blue>发送的信息条数：</font> <%= session.getAttribute("change")%> </td>
    </tr>
  </table>
</FORM>
