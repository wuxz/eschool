...
public class ScreenFlowXmlDAO
{
    ...
    public static Element loadDocument(String location)
    {
        ...
        {
            System.out.println ("ScreenFlowXmlDAO ** Parsing error" + ", line " +
                        err.getLineNumber () + ", uri " + err.getSystemId ());
            System.out.println("ScreenFlowXmlDAO error: " + err.getMessage ());
        } catch (SAXException e)
        {
            System.out.println("ScreenFlowXmlDAO error: " + e);
        } catch (java.net.MalformedURLException mfx)
        {
            System.out.println("ScreenFlowXmlDAO error: " + mfx);
        } catch (java.io.IOException e)
        {
            System.out.println("ScreenFlowXmlDAO error: " + e);
        } catch (Exception pce)
        {
            System.out.println("ScreenFlowXmlDAO error: " + pce);
        }
        return null;
    }
    ...
    public static ScreenFlowData loadScreenFlowData(String location)
    {
        ...
        String teacherHome  = getTagValue(root, TEACHER_HOME);
        String studentHome  = getTagValue(root, STUDENT_HOME);
        String adminHome  = getTagValue(root, ADMIN_HOME);
    ...
    public static HashMap getScreenDefinitions(Element root)
    {
        ...
        for (int loop = 0; loop < list.getLength(); loop++)
        {
            ...
            if (node != null)
            {
                ...
                {
                    System.out.println("*** Non Fatal errror: ScreenDefinitions for language " + language +
    ...
    public static HashMap getScreens(Element root)
    {
        ...
        for (int loop = 0; loop < list.getLength(); loop++)
        {
            ...
            if (node != null)
            {
                ...
                {
                    System.out.println("*** Non Fatal errror: Screen " + screenName +
                                       " defined more than once in screen definitions file");
                }
            }
        }
            ...
            for (int innerLoop =0; innerLoop < children.getLength(); innerLoop++)
            {
                ...
                if ((child != null) && (child.getNodeName() != null) && child.getNodeName().equals(PARAMETER) )
                {
                    if (child instanceof Element)
                    {
                        ...
                        {
                            System.out.println("*** Non Fatal errror: " +
        ...
        for (int loop = 0; loop < list.getLength(); loop++)
        {
            ...
            if (node != null)
            {
                ...
                if (node instanceof Element)
                {
                    ...
                    popedomString = element.getAttribute(POPEDOM);
                }
                if (popedomString != null) popedom=popedomString;
                ...
                if (useFlowHandler)
                {
                    ...
                    if (node instanceof Element)
                    {
                        ...
                        if (children.getLength() > 1)
                        {
                                 System.out.println("Non fatal error: There can be only one <" + FLOW_HANDLER +
                            ...
                            if (flowHandlerNode instanceof Element)
                            {
                                ...
                                for (int resultLoop=0; resultLoop < results.getLength(); resultLoop++)
                                {
                                    ...
                                    if (resultNode instanceof Element)
                                    {
                                        ...
                                        {
                                            System.out.println("*** Non Fatal errror: Screen " + url + " <" + FLOW_HANDLER +
                ...
                {
                    System.out.println("*** Non Fatal errror: Screen " + url +
                                       " defined more than once in screen definitions file");
                }
            }
        }
